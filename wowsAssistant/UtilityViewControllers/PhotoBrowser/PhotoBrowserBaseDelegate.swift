//
//  PhotoBrowserBaseDelegate.swift
//  AppFunFeautres
//
...
open class PhotoBrowserBaseDelegate: NSObject, PhotoBrowserDelegate {
    ...
    open var longPressedCallback: ((PhotoBrowser, Int, UIImage?, UILongPressGestureRecognizer) -> Void)?
    ...
    open func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        guard let cell = cell as? PhotoBrowserBaseCell else {
            return
        }
    ...
    open func displayingContentView(_ browser: PhotoBrowser, pageIndex: Int) -> UIView? {
        let indexPath = IndexPath.init(item: pageIndex, section: 0)
        let cell = browser.collectionView.cellForItem(at: indexPath) as? PhotoBrowserBaseCell
        return cell?.imageView
    }
    
    /// 取转场动画视图
    open func transitionZoomView(_ browser: PhotoBrowser, pageIndex: Int) -> UIView? {
        let indexPath = IndexPath.init(item: pageIndex, section: 0)
        let cell = browser.collectionView.cellForItem(at: indexPath) as? PhotoBrowserBaseCell
        return UIImageView(image: cell?.imageView.image)
    }
    
    open func photoBrowser(_ browser: PhotoBrowser, pageIndexDidChanged pageIndex: Int) {
        // Empty.
    }
    
    open func photoBrowserViewDidLoad(_ browser: PhotoBrowser) {
        // Empty.
    }
    
    open func photoBrowser(_ browser: PhotoBrowser, viewWillAppear animated: Bool) {
        // 遮盖状态栏
        coverStatusBar(true)
    }
    
    open func photoBrowserViewWillLayoutSubviews(_ browser: PhotoBrowser) {
        // Empty.
    }
    
    open func photoBrowserViewDidLayoutSubviews(_ browser: PhotoBrowser) {
        // Empty.
    }
    
    open func photoBrowser(_ browser: PhotoBrowser, viewDidAppear animated: Bool) {
        // Empty.
    }
    
    open func photoBrowser(_ browser: PhotoBrowser, viewWillDisappear animated: Bool) {
        // 还原状态栏
        coverStatusBar(false)
    }
    
    open func photoBrowser(_ browser: PhotoBrowser, viewDidDisappear animated: Bool) {
        // Empty.
    }
    
    open func dismissPhotoBrowser(_ browser: PhotoBrowser) {
        self.dismiss()
    }
    
    open func photoBrowserDidReloadData(_ browser: PhotoBrowser) {
        // Empty.
    }
}
