//
//  PhotoBrowserZoomTransition.swift
//  AppFunFeautres
//
...
public class PhotoBrowserZoomTransitioning: PhotoBrowserTransitioning {
    /// present转场时，内容缩张模式
    ...
    public convenience init(originViewCallback: @escaping ViewClosure) {
        let callback: FrameClosure = { (browser, index, view) -> CGRect? in
            if let oriView = originViewCallback(browser, index, view) {
                // 对`PhotoBrowserZoomTransitioningOriginResource`调用对应实现
                if let oriRes = oriView
                    as? PhotoBrowserZoomTransitioningOriginResource {
                    return PhotoBrowserZoomTransitioning.resRect(oriRes: oriRes,
                                                                   to: view)
                }
    ...
    public typealias ResourceClosure = (
        _ browser: PhotoBrowser,
        _ pageIndex: Int,
        _ transContainer: UIView) -> PhotoBrowserZoomTransitioningOriginResource?
    
    /// 初始化，传入动画"起始/结束"的前置视图，
    /// 要求实现了`PhotoBrowserZoomTransitioningOriginResource`，如UIImageView.
    public convenience init(originResourceCallback: @escaping ResourceClosure) {
        let callback: FrameClosure = { (browser, index, view) -> CGRect? in
            if let oriRes = originResourceCallback(browser, index, view) {
                return PhotoBrowserZoomTransitioning.resRect(oriRes: oriRes, to: view)
            }
            return nil
        }
        self.init(originFrameCallback: callback)
    ...
    private func setupPresenting() {
        weak var `self` = self
        presentingAnimator = PhotoBrowserZoomPresentingAnimator(zoomView: { () -> UIView? in
            guard let `self` = self else {
                print("PhotoBrowserZoomTransitioning 已被释放.")
                return nil
            }
    ...
    private func setupDismissing() {
        weak var `self` = self
        dismissingAnimator = PhotoBrowserZoomDismissingAnimator(zoomView: { () -> UIView? in
            guard let `self` = self else {
                print("PhotoBrowserZoomTransitioning 已被释放.")
                return nil
            }
